...
-- ============================================================================
-- gen_random_uuid() متوفرة افتراضياً في PostgreSQL 13+ (Supabase)
...
-- ⚠️ تم نقلها إلى migration منفصل (20260309000002) بعد تفعيل pgvector من Dashboard
-- ============================================================================
...
-- دالة get_ai_recommendations تم نقلها إلى migration منفصل (20260309000002)
-- بعد تفعيل pgvector من Supabase Dashboard
...
BEGIN
    ...
    IF claims->'app_metadata' IS NULL THEN
        claims := jsonb_set(claims, '{app_metadata}', jsonb_build_object('user_role', v_user_role, 'permissions', v_permissions));
    ELSE
        claims := jsonb_set(claims, '{app_metadata, user_role}', to_jsonb(v_user_role));
        claims := jsonb_set(claims, '{app_metadata, permissions}', v_permissions);
    END IF;
...
-- user_embeddings و item_embeddings: RLS في migration منفصل (20260309000002)
...
-- سياسات RLS في migration منفصل (20260309000002) بعد تفعيل pgvector
...
-- ⚠️ تم نقلها إلى migration منفصل (20260309000003) بعد تفعيل pg_cron من Dashboard
-- ============================================================================
